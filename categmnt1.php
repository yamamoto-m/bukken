...
/****************************************/
  ...
  if (!session_is_registered($ADMSESS)) {
    ...
    //ログインページへジャンプします
    header("location: adminlogin.php?id=4");
    exit();
  }
    ...
    for ($cnt = 0; $cnt < $reccnt; $cnt++) {
      ...
      if (isset($_POST[$btnname])) {
        ...
        $sql = "UPDATE tblcategory
                SET categoryname1 = \"" . $_POST[categname1][$cnt] . "\"
                WHERE categoryid1 = "   . $_POST[categid1][$cnt];
        //SQL文を発行します
        mysql_query($sql, $con);
      }
      //削除ボタンをチェックします
...
　　　　　unlink("$PHOTODIR$col[madoriimagefile]");
    ...
    if (isset($_POST[$btnname])) {
      ...
      $sql = "INSERT INTO tblcategory
              (categoryname1) VALUES (\"$_POST[newcateg]\")";
      //SQL文を発行します
      mysql_query($sql, $con);
    }
  }
  ...
  $sql = "SELECT * FROM tblcategory ORDER BY categoryid1";
